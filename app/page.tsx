  ...
  return (
    ...
    >
      ...
      <section className="messages" id="memories">
        ...
        {loadingNotes ? <div className="recipient-loading" aria-label="Loading postcards"><i/><i/><i/></div> : noteLoadError ? <div className="recipient-empty"><span>◯</span><p>We couldn’t gather the postcards just yet.</p><button onClick={() => window.location.reload()}>Try again</button></div> : notes.length === 0 ? <div className="recipient-empty"><span>◯</span><p>The first postcard is still being written.</p></div> : <div className="cards">{notes.map((note, i) => <article className={`note note-${(i % 3) + 1}${note.media?.length ? " note-has-media" : ""}`} key={note.id ?? `${note.name}-${i}`} tabIndex={0} role="button" aria-label={`Open postcard ${i + 1} of ${notes.length} from ${note.name}`} onClick={() => openPostcard(i)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openPostcard(i); } }} onPointerEnter={sound.playPaper}><div className="stamp">{note.stamp}</div><div className="postmark" aria-hidden="true">◯<br/><span>20.08.26</span></div><span className="quote" aria-hidden="true">“</span><p>{note.text}</p>{note.media?.length > 0 && <PostcardMediaGallery items={note.media} variant="recipient" label="Postcard photos and GIFs" lazy/>}{note.stickers?.length > 0 && <div className="postcard-sticker-row" aria-label="Postcard stickers">{note.stickers.map((sticker, index) => <span key={`${sticker.symbol}-${index}`} title={sticker.label}>{sticker.symbol}</span>)}</div>}<footer><div><b>{note.name}</b><small>{note.role}</small></div><span className="note-open-hint">Open postcard <i aria-hidden="true">↗</i></span></footer></article>)}</div>}
