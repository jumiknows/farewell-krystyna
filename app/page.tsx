      ...
      {selectedNoteIndex !== null && notes[selectedNoteIndex] && <div className="note-reader" role="dialog" aria-modal="true" aria-labelledby="focused-postcard-author" onMouseDown={event => { if (event.target === event.currentTarget) setSelectedNoteIndex(null); }}>
        ...
        <div className={notes.length === 1 ? "reader-stage is-single" : "reader-stage"}>
          ...
          <article className="focused-postcard" key={readerTurn}>
            ...
            <div className="focused-postcard-body"><p>{notes[selectedNoteIndex].text}</p>{notes[selectedNoteIndex].media?.length > 0 && <div className="postcard-media-grid focused-media" aria-label="Postcard photos and GIFs">{notes[selectedNoteIndex].media.map(item => <img key={item.src} src={item.src} alt={item.label}/>)}</div>}{notes[selectedNoteIndex].stickers?.length > 0 && <div className="postcard-sticker-row focused-stickers" aria-label="Postcard stickers">{notes[selectedNoteIndex].stickers.map((sticker, index) => <span key={`${sticker.symbol}-${index}`} title={sticker.label}>{sticker.symbol}</span>)}</div>}</div>
