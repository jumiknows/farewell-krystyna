...
export function ParisMagic({ active, finale }: { active: boolean; finale: number }) {
  ...
  useEffect(() => {
    ...
    void import("three").then(THREE => {
      ...
      const current = new Float32Array(count * 3), drift = new Float32Array(count * 3), tower = new Float32Array(count * 3), heart = new Float32Array(count * 3), colours = new Float32Array(count * 3);
      ...
      for (let i = 0; i < count; i++) {
        ...
        heart[k] = Math.sin(a) ** 3 * 1.75; heart[k + 1] = (13 * Math.cos(a) - 5 * Math.cos(2*a) - 2 * Math.cos(3*a) - Math.cos(4*a)) * .12; heart[k + 2] = (Math.random() - .5) * .35;
      ...
      let pointerX=0,pointerY=0,frame=0,lastFinale=finaleRef.current,heartUntil=0;
      ...
      const animate=()=>{
        const t=clock.getElapsedTime(); if(lastFinale!==finaleRef.current){lastFinale=finaleRef.current;heartUntil=t+5.2}
        ...
        const heartMix=t<heartUntil?Math.min(1,(heartUntil-t)/.8):0, target=heartMix>.02?heart:tower, formation=heartMix>.02?heartMix:parisMix;
        ...
        const travel=((scrollY/Math.max(1,document.documentElement.scrollHeight-innerHeight))*.82+t*.025)%1;plane.position.copy(path.getPoint(travel));const tangent=path.getTangent(travel);plane.rotation.z=Math.atan2(tangent.y,tangent.x);pathLine.visible=parisMix<.78&&heartMix<.02;plane.visible=pathLine.visible;
